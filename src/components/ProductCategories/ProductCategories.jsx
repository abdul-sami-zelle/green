...
import { useRouter } from "next/navigation";
import { getCategories, apiUrl } from "../../lib/api";
import "./style.css";
...
return (
  <div className="product-categories-main">
    <div className="product-categories-first">
      {loading
        ? [...Array(10)].map((_, i) => (
            <div key={i} className="product-categories-second shimmer-card">
              <div className="image-wrapper shimmer-image"></div>
              <div className="category-text-container">
                <span className="category-text shimmer-text">&nbsp;</span>
              </div>
            </div>
          ))
        : categories.map((cat) => (
